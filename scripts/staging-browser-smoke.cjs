...
async function main() {
  ...
  try {
    ...
    const registerForm = page.locator('form.auth-form');
    ...
    await page.locator('input[name="name"]').fill('Browser Native Acceptance');
    await page.locator('input[name="organizationName"]').fill('Browser Native Acceptance Org');
    ...
    if (finalUrl.pathname !== '/dashboard' || finalUrl.search) {
      throw new Error(`Native registration ended at unexpected URL ${page.url()}`);
    }
    ...
    await assertNoHorizontalOverflow(page, 'dashboard/desktop/de/native-no-js', 1440);
    if (serverErrors.length) throw new Error(`dashboard server errors ${serverErrors.join(',')}`);
