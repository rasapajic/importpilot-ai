...
function buildGeneralPrompt(input: SearchRequest, maxResults: number) {
  return [
    ...
    "When an already opened cited page exposes a direct URL for the main product photo, return it as imageUrl. Never use a marketplace logo, supplier logo, avatar, icon, flag, badge or placeholder as the product image.",
...
function buildAlibabaOnlyPrompt(input: SearchRequest, maxResults: number) {
  return [
    ...
    "When the cited page exposes a direct URL for the main product photo, return it as imageUrl. Never use an Alibaba logo, company logo, avatar, icon, flag, badge or placeholder as the product image.",
    "Use a two-letter supplier-country code only when confirmed; otherwise null.",
